...
def _transcribe_groq(temp_path: str, mime_type: str) -> tuple[str, str]:
...
def _run_provider(provider: str, temp_path: str, mime_type: str) -> tuple[str, str]:
    try:
        if provider == "khaya":
            return _transcribe_khaya(temp_path)
        return _transcribe_groq(temp_path, mime_type)
    except Exception:
        return "", "unknown"
...
def transcribe_from_audio_url(audio_url: str, access_token: str | None = None) -> tuple[str, str]:
    ...
    _suffix, mime_type = _audio_mime(audio_url)
    try:
        text, language = _run_provider(_primary_provider(), temp_path, mime_type)
        if not text:
            text, language = _run_provider(_fallback_provider(), temp_path, mime_type)
        if not text:
